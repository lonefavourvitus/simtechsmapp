<script>
	...
	import { drugStore, drugProperties, trigAngRotOnHover, darkMode } from '../stores';
	import ToolTip from '$lib/ToolTip.svelte';
	import Angle from '/src/icons/Angle.svelte';
	import { fade, blur, fly, slide, scale } from 'svelte/transition';
	// let darkModer = $darkMode;
	let isHovered = false;
	...
	function handleHover() {
		...
		isHovered = true;
		$trigAngRotOnHover = true;
	}

	function handleMouseLeave() {
		hideTimeout = setTimeout(() => {
			isHovered = false;
			$trigAngRotOnHover = false;
...
>
	...
	{#if isHovered}
		<main
			transition:fly
			on:mouseenter={handleHoverForMenuItems()}
